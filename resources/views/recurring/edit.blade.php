...
<div class="max-w-5xl mx-auto space-y-8" 
     x-data="{
        ...
        discountRate: {{ (float)$recurring->discount_rate }},
        currency: '{{ old('currency', $recurring->currency) }}',
        selectedStyle: '{{ old('style', $recurring->style) }}',
        selectedLogoId: {{ old('logo_id', $recurring->logo_id ?: 'null') }},
        ...
        items: {{ json_encode($recurring->items->map(fn($item) => [
        ...
        additionalCharges: {{ json_encode($recurring->additional_charges ?: []) }},
        addAdditionalCharge() {
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-extrabold text-slate-900 dark:text-white flex items-center gap-2">
                <i data-lucide="edit" class="w-6 h-6 text-indigo-600 dark:text-indigo-400"></i>
                Edit Recurring Profile: {{ $recurring->title }}
            </h2>
            <p class="text-sm text-slate-500 dark:text-slate-400">Update recurrence frequency, items, pricing, or billing terms</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('recurring.show', $recurring) }}" class="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                Cancel
            </a>
            <button type="submit" form="recurring-edit-form" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm shadow-sm transition">
    ...
    <form id="recurring-edit-form" action="{{ route('recurring.update', $recurring) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $recurring->title) }}" required
                ...
                <div>
                    ...
                    <select name="frequency" required class="w-full px-4 py-2.5 rounded-xl text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none font-medium">
                        <option value="weekly" {{ old('frequency', $recurring->frequency) === 'weekly' ? 'selected' : '' }}>Weekly (Every 7 days)</option>
                        <option value="biweekly" {{ old('frequency', $recurring->frequency) === 'biweekly' ? 'selected' : '' }}>Bi-Weekly (Every 14 days)</option>
                        <option value="monthly" {{ old('frequency', $recurring->frequency) === 'monthly' ? 'selected' : '' }}>Monthly (Default)</option>
                        <option value="quarterly" {{ old('frequency', $recurring->frequency) === 'quarterly' ? 'selected' : '' }}>Quarterly (Every 3 months)</option>
                        <option value="yearly" {{ old('frequency', $recurring->frequency) === 'yearly' ? 'selected' : '' }}>Yearly (Annual renewal)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date', $recurring->end_date?->format('Y-m-d')) }}"
                           class="w-full px-4 py-2.5 rounded-xl text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="due_days" value="{{ old('due_days', $recurring->due_days) }}" min="1" max="180" required
                           class="w-full px-4 py-2.5 rounded-xl text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 dark:border-slate-800">
                <label class="relative flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" name="auto_send_email" value="1" {{ old('auto_send_email', $recurring->auto_send_email) ? 'checked' : '' }} class="mt-1 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                    <div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500 dark:text-slate-400">Click any product from your catalog to add into line items</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-3">
                ...
                <textarea name="notes" rows="4" placeholder="Included in every generated invoice..."
                          class="w-full px-3.5 py-2.5 rounded-xl text-xs sm:text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">{{ old('notes', $recurring->notes) }}</textarea>
            </div>
            ...
            <div class="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-3">
                ...
                <textarea name="payment_instructions" rows="4" placeholder="Bank details, wire instructions or payment notes for client..."
                          class="w-full px-3.5 py-2.5 rounded-xl text-xs sm:text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">{{ old('payment_instructions', $recurring->payment_instructions) }}</textarea>
            </div>
        </div>

        <div class="flex justify-end gap-3 pt-4">
            <a href="{{ route('recurring.show', $recurring) }}" class="px-5 py-2.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                Cancel
            </a>
