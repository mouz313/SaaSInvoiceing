...
<head>
    ...
    <title>Create Account — InvoiceHub</title>
...
<body class="h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <h2 class="mt-4 text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Start invoicing in seconds</h2>
        <p class="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">Get 5 free invoices automatically upon sign up</p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md px-4">
        <div class="bg-white dark:bg-slate-900 py-8 px-6 sm:px-10 shadow-xl border border-slate-200 dark:border-slate-800 rounded-3xl space-y-6">
            ...
            <!-- 5 Free Invoices Callout -->
            <div class="p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/60 flex items-center gap-2.5 text-xs text-emerald-800 dark:text-emerald-300 font-medium">
                <i data-lucide="gift" class="w-4 h-4 text-emerald-600 shrink-0"></i>
                <span>Special Promo: <strong>5 Free Invoice Credits</strong> granted instantly!</span>
            </div>
            ...
            <form method="POST" action="{{ route('register.post') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required autofocus placeholder="Alex Rivera"
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="alex@studio.com"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-blue-600 focus:outline-none">
            ...
            <p class="text-center text-xs text-slate-500 dark:text-slate-400">
                ...
                <a href="{{ route('login') }}" class="font-bold text-blue-600 dark:text-blue-400 hover:underline">Sign in &rarr;</a>
            </p>
        </div>
    </div>
    ...
    <script>
        ...
        googleBtn?.addEventListener('click', async () => {
            ...
            try {
                ...
                alert(messages[error.code] || 'Google Sign-Up failed: ' + error.message);
            }
        });
    </script>
</body>
