...
<body class="bg-slate-100 dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-full flex flex-col antialiased">
    ...
    <main class="flex-1 py-8 sm:py-12 px-4 sm:px-6">
        ...
        <div class="max-w-4xl mx-auto mt-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
        ...
        @if(!$invoice->isPaid())
        <div class="max-w-4xl mx-auto mt-6 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/80 rounded-2xl p-6"
             x-data="{
            ...
            <form method="POST" action="{{ route('invoices.public.checkout', $invoice->public_token) }}" class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto justify-end">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-sm transition flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap">
                        <i data-lucide="lock" class="w-3.5 h-3.5"></i>
                        <span x-text="'Checkout ' + '{{ $invoice->currency }} ' + (parseFloat(payAmount) || 0).toFixed(2)"></span>
                    </button>
                </div>
            </form>
        </div>
