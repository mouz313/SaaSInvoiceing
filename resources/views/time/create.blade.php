...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white">Log Time Entry</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Record billable task hours for your clients</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 shadow-xs">
        <form method="POST" action="{{ route('time.store') }}" class="space-y-4" x-data="{ hours: 1, rate: 50 }">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="project_name" value="{{ old('project_name') }}" placeholder="e.g. Website Redesign"
            ...
            <div>
                ...
                <textarea name="task_description" rows="3" required placeholder="Describe what was accomplished..."
                          class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-blue-600 focus:outline-none">{{ old('task_description') }}</textarea>
            </div>
