...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white">Edit Expense</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Update expense details</p>
        </div>
        ...
        <form method="POST" action="{{ route('expenses.update', $expense) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="client_id" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:ring-2 focus:ring-blue-600 focus:outline-none">
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('client_id', $expense->client_id) == $client->id ? 'selected' : '' }}>{{ $client->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="pt-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_billable" value="1" {{ old('is_billable', $expense->is_billable) ? 'checked' : '' }} class="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 border-slate-300">
